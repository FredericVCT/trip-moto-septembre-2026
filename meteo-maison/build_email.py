...
def compute():
    ...
    for day in sorted(set(t[:10] for t in T)):
        ...
        rows.append(dict(day=day, p1=p1, med=round(med, 1), p90=round(p90, 1), bl=bl, det=det, tmin=round(min(tt)), tmax=round(max(tt)), gust=round(g), n=len(tots)))
    return rows
...
def render():
    ...
    for r in rows:
        ...
        H.append(f'<tr><td style="padding:6px 10px;border-top:1px solid #eef2f6"><b>{JOURS[dd.weekday()]} {dd.day:02d}/{dd.month:02d}</b><div style="font-size:11.5px;color:#5a6b7c">{C[r["day"]][1]}</div></td><td {td}><b>{r["tmin"]:.0f} à {r["tmax"]:.0f}°</b></td><td {td}>{pill(str(r["p1"]) + " %", r["p1"])}</td><td {td}>{cum}</td><td {td}>{r["gust"]:.0f} km/h</td></tr>')
    H.append('</table><div style="font-size:11px;color:#5a6b7c;padding:6px 10px 8px">Risque : part des scénarios d\'ensemble (ECMWF, AIFS, ICON, GFS) qui donnent au moins 1 mm dans la journée. Cumul : valeur médiane, et entre parenthèses le cumul atteint dans les 10 % de scénarios les plus pluvieux.</div></div>')
    ...
    for r in [x for x in rows if x["p90"] >= .2]:
        dd = datetime.date.fromisoformat(r["day"])
...
def summary():
    ...
    print("jour        T°      P>=1mm  médiane  p90   rafales  | nuit/matin/aprem/soir (P>=0,5mm)  | modèles (mm)")
    for r in rows:
        print(f"{r['day']}  {r['tmin']:>2}/{r['tmax']:<2}°  {r['p1']:>4} %  {r['med']:>5}   {r['p90']:>5}  {r['gust']:>3} km/h | " + "/".join(str(v) for v in r['bl'].values()) + " | " + " · ".join(r['det']))
    if os.path.exists(ps):
        ...
        out["changes"] = lines
        print(f"\nÉcarts depuis le briefing du {old['issued']} (seuils : 20 pts, 2 mm, 3°) :")
